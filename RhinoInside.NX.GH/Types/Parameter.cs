...
namespace RhinoInside.NX.GH.Types
{
  public class ParameterKey : Element
  {
    ...
    public ParameterKey() { }
    public ParameterKey(DB.Part doc, DB.Tag id) : base(doc, id) { }
    ...
    new class Proxy : Element.Proxy
    {
      ...
      DB.BuiltInParameter builtInParameter => owner.Tag.TryGetBuiltInParameter(out var bip) ? bip : DB.BuiltInParameter.INVALID;
      DB.ParameterElement parameter => IsBuiltIn ? null : owner.Document?.GetElement(owner.Tag) as DB.ParameterElement;
    ...
    public override string DisplayName
    {
      get
      {
        try
        {
          if (Tag is object && Tag.TryGetBuiltInParameter(out var builtInParameter))
