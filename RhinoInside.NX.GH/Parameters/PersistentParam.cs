...
{
    public abstract class PersistentGeometryParam<T> : GH_PersistentParam<T>
      where T : class, IGH_Goo
    {
        protected override sealed Bitmap Icon
        {
            get => ((Bitmap)Properties.Resources.ResourceManager.GetObject(GetType().Name)) ?? ImageBuilder.BuildIcon(IconTag, Properties.Resources.UnknownIcon);
        }

        protected virtual string IconTag => typeof(T).Name.Substring(0, 1);
        public virtual void SetInitCode(string code) => NickName = code;

        protected PersistentGeometryParam(string name, string nickname, string description, string category, string subcategory) :
