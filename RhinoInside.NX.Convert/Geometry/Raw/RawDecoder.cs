...
namespace RhinoInside.NX.Convert.Geometry.Raw
{
    /// <summary>
    ...
    static class RawDecoder
    {
        ...
        public static Point3d AsPoint3d(NXOpen.Point3d p)
        {
            return new Point3d(p.X, p.Y, p.Z);
        }
        public static Vector3d AsVector3d(NXOpen.Vector3d p)
        {
            return new Vector3d(p.X, p.Y, p.Z);
        }

        public static Rhino.Geometry.Transform AsTransform(NXOpen.Matrix4x4 transform)
        {
        ...
        public static Plane AsPlane(NXOpen.Plane plane)
        {
            return new Plane(AsPoint3d(plane.Origin), AsVector3d(plane.Matrix.GetAxisX()), AsVector3d(plane.Matrix.GetAxisY()));
        }
        #endregion
        ...
        {
            return new Point(AsPoint3d(point.Coordinates));
        }
        #endregion

        #region Curve
        public static LineCurve ToRhino(NXOpen.Line line)
        {
            return new LineCurve(new Line(AsPoint3d(line.StartPoint), AsPoint3d(line.EndPoint)));
        }

        public static ArcCurve ToRhino(NXOpen.Arc arc)
        {
            return new ArcCurve(new Circle(new Plane(AsPoint3d(arc.CenterPoint), AsVector3d(arc.Matrix.Element.GetAxisX()), AsVector3d(arc.Matrix.Element.GetAxisY())), arc.Radius), arc.StartAngle, arc.EndAngle);
        }

        public static NurbsCurve ToRhino(NXOpen.Ellipse ellipse)
        {
            var plane = new Plane(AsPoint3d(ellipse.CenterPoint), AsVector3d(ellipse.Matrix.Element.GetAxisX()), AsVector3d(ellipse.Matrix.Element.GetAxisY()));
            ...
            {
                nurbsCurve.ClosestPoint(AsPoint3d(ellipse.GetPoint(0.0)), out var param0);
                if (!nurbsCurve.ChangeClosedCurveSeam(param0))
                    nurbsCurve.Domain = new Interval(param0, param0 + nurbsCurve.Domain.Length);

                nurbsCurve.ClosestPoint(AsPoint3d(ellipse.GetPoint(1.0)), out var param1);
        ...
        public static NurbsCurve ToRhino(NXOpen.Spline spline)
        {
        ...
        public static Curve ToRhino(NXOpen.Curve curve)
        {
            switch (curve)
            {
                ...
                case NXOpen.Line line: return ToRhino(line);
                case NXOpen.Arc arc: return ToRhino(arc);
                case NXOpen.Ellipse ellipse: return ToRhino(ellipse);
                case NXOpen.Spline nurb: return ToRhino(nurb);
                default: throw new NotImplementedException();
            }
        }

        public static PolylineCurve ToRhino(NXOpen.Polyline polyline)
        {
            return new PolylineCurve(polyline.GetPoints().Select(x => AsPoint3d(x)));
        }
        #endregion
        ...
        public static Surface ToRhinoSurface(NXOpen.Face face, out bool parametricOrientation, double relativeTolerance = 0.0)
        {
            ...
            {
                case NXOpen.Face.FaceType.Planar: return FromPlane(face, relativeTolerance);
                case NXOpen.Face.FaceType.Conical: return FromConicalSurface(face, relativeTolerance);
        ...
        static PlaneSurface FromPlane(NXOpen.Face face, double relativeTolerance)
        {
        ...
        static RevSurface FromConicalSurface(NXOpen.Face face, double relativeTolerance)
        {
            var bboxUV = face.GetUVBoundingBox();
            var faceData = face.GetData();
            ...
            var plane = new Plane(AsPoint3d(faceData.Point), AsVector3d(faceData.Direction));

            var dir = AsVector3d(faceData.Direction) + Math.Tan(faceData.RadiusData) * plane.XAxis;
            dir.Unitize();
        ...
        static RevSurface FromSphereSurface(NXOpen.Face face, double relativeTolerance)
        {
            ...
            return RevSurface.CreateFromSphere(new Sphere(AsPoint3d(faceData.Point), faceData.Radius));
        }
        ...
        static RevSurface FromCylindricalSurface(NXOpen.Face face, double relativeTolerance)
        {
            ...
            (
              new Line
              (
                plane.Origin + (faceData.Radius * plane.XAxis) + (vv.Min * AsVector3d(faceData.Direction)),
                plane.Origin + (faceData.Radius * plane.XAxis) + (vv.Max * AsVector3d(faceData.Direction))
              ),
              vv.Min,
              vv.Max
            );
