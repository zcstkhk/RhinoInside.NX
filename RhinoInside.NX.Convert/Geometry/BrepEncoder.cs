...
namespace RhinoInside.NX.Convert
{
    ...
    {
        private static NXOpen.UF.UFSession theUfSession = NXOpen.UF.UFSession.GetUFSession();
                ...
                foreach (var face in brep.Faces)
                {
                    using (var nurbSurface = face.ToNurbsSurface())
                    {
                        ...
                        if (nurbSurface.IsPlanar())
                        {
                            ...
                            var boundedPlaneBody = WorkPart.Features.CreateBoundedPlane(boundaryCurves).GetBodies()[0];
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < boundaryEdgeCurves.Count; i++)
                            {
                                ...
                                theUfSession.Modl.TrimSheet(faceBody.Tag, trimBoundaryObjects.Length, trimBoundaryObjects, null, 1, 1, point3dOnFace.ToXYZ().ToArray(), 0.01, out int numGapPoints, out double[] gapPoints);
                            }
